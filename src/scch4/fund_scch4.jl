...
#function fund_damages(base_temp::Array{Union{Missing,Float64},2}, base_co2::Array{Union{Missing,Float64},2}, pulse_temp::Array{Union{Missing,Float64},2}, pulse_co2::Array{Union{Missing,Float64},2}, end_year::Int)
function fund_damages(base_temp::Array{Float64,2}, base_co2::Array{Float64,2}, pulse_temp::Array{Float64,2}, pulse_co2::Array{Float64,2}, end_year::Int)
           ...
           #Find indices in climate results that correspond to FUND years (assuming SNEASY+CH4 initialized in 1765).
           ...
           #Caluclate number of times to run FUND (equal to number of posterior parameter samples used).
           ...
           # Pre-allocate arrays to store FUND output (note, FUND has 16 regions).
           ...
           for i in 1:number_samples
            ...
            try
               ...
               # FUND cannot accept negative temperature values (which may occur in a few early periods due to superimposed noise). In both IAMs, set negative anomolies to 0.0 to maintain consistency.
                ...
                #Calculate annual marginal damages (note: 1950 damage = `missing`, so set to 0.0 for convenience).
...
function fund_scch4(marginal_damages::Array{Float64,3}, regional_consumption::Array{Float64,3}, regional_population::Array{Float64,3}, pulse_year::Int, end_year::Int; constant::Bool=true, η::Float64=1.5, γ::Float64=1.5, ρ::Float64=0.01, dollar_conversion::Float64=1.35, equity_weighting=false)
    ...
    # Calculate the SC-CH4 for each posterior parameter sample.
    ...
    elseif equity_weighting == true
        ...
        # Following the approach of Anthoff & Emmerling (2019), calculate equity-weighted SC-CH4 values for each normalization region (this starts discounting at year of pulse).
        for i = 1:number_samples
            for r = 1:16

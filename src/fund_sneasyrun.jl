...
function constructfund(;nsteps=1050)
    ...
    #Fake test data.
   # srand(123);
   # temp = Float64[0.8 * (1+0.0025)^t + rand(Normal(0., 0.1), 1)[1] for t = 1:nsteps+1];

   # srand(123);
   # acco2 = Float64[280 * (1+0.00198)^t + rand(Normal(0., 0.1), 1)[1] for t = 1:nsteps+1];


    # ------------------------------------------------------------------------------------
    # Set CO2 and global temperature parameters that are coming from SNEASY climate model.
    # ------------------------------------------------------------------------------------

    #Temperature Parameters
    setparameter(m, :climateregional, :inputtemp, zeros(nsteps+1))
    setparameter(m, :biodiversity, :temp, zeros(nsteps+1))
    setparameter(m, :ocean, :temp, zeros(nsteps+1))

    #CO2 Concnetration Parameters
    setparameter(m, :impactagriculture, :acco2, zeros(nsteps+1))
    setparameter(m, :impactextratropicalstorms, :acco2, zeros(nsteps+1))
    setparameter(m, :impactforests, :acco2, zeros(nsteps+1))
...
function getfund(;nsteps=1050, datadir="src/fund/data", params=nothing)
    # ---------------------------------------------
    # Load parameters
    # ---------------------------------------------
    if params==nothing

...
include("src/sneasy_hector/sneasy_hector.jl")
#include("src/sneasy_hector/sneasy_hector_oldRF.jl")
include("calibration/calibration_helper_functions.jl")

#mcmc_data ="UNIFORM_ECS_5MILL"
...
function climate_h(mcmc_data::String, chain_size::String; mean_chain::Bool=false, maxpost_chain::Bool=false, Conf_lvl_1::Float64=0.90, Conf_lvl_2::Float64=0.98)
    ...
    #else
        chain = DataFrame(load(joinpath("calibration/sneasy_hector/mcmc_results/", mcmc_data, string(chain_size, ".csv"))))
    ...
    sneasy_base = getsneasy_hector(rcp_scenario= rcp_scenario, start_year=start_year, end_year=end_year);
    sneasy_marginal = getsneasy_hector(rcp_scenario =rcp_scenario, start_year=start_year, end_year=end_year);
    ...
    setparameter(sneasy_marginal, :ch4cyclehector, :CH4_emissions, ch4_marginal)
    ...
    for i in 1:number_samples
        ...
        setparameter(sneasy_base, :ohcyclehector, :TOH0, TOH0)
        setparameter(sneasy_base, :ohcyclehector, :M0, CH4_0)
        setparameter(sneasy_base, :rfch4hector, :CH4_0, CH4_0)
        setparameter(sneasy_base, :rfch4hector, :N₂O_0, N2O_0)
        setparameter(sneasy_base, :rfch4hector, :scale_CH₄, rf_scale_CH₄)
        ...
        setparameter(sneasy_base, :ch4cyclehector, :M0, CH4_0)
        setparameter(sneasy_base, :ch4cyclehector, :CH4N, CH4_Nat)
        setparameter(sneasy_base, :ch4cyclehector, :Tsoil, Tsoil)
        setparameter(sneasy_base, :ch4cyclehector, :Tstrat, Tstrat)
        setparameter(sneasy_base, :rfch4h2ohector, :M0, CH4_0)
        ...
        setparameter(sneasy_marginal, :ohcyclehector, :TOH0, TOH0)
        setparameter(sneasy_marginal, :ohcyclehector, :M0, CH4_0)
        setparameter(sneasy_marginal, :rfch4hector, :CH4_0, CH4_0)
        setparameter(sneasy_marginal, :rfch4hector, :N₂O_0, N2O_0)
        setparameter(sneasy_marginal, :rfch4hector, :scale_CH₄, rf_scale_CH₄)
        ...
        setparameter(sneasy_marginal, :ch4cyclehector, :M0, CH4_0)
        setparameter(sneasy_marginal, :ch4cyclehector, :CH4N, CH4_Nat)
        setparameter(sneasy_marginal, :ch4cyclehector, :Tsoil, Tsoil)
        setparameter(sneasy_marginal, :ch4cyclehector, :Tstrat, Tstrat)
        setparameter(sneasy_marginal, :rfch4h2ohector, :M0, CH4_0)
        ...
        base_ch4[i,:]                 = sneasy_base[:ch4cyclehector, :CH4] .+ ar1_hetero_ch4
        ...
        #base_lifech4[i,:]             = sneasy_base[:ohcyclehector, :TOH]
        ...
        marginal_ch4[i,:]             = sneasy_marginal[:ch4cyclehector, :CH4] .+ ar1_hetero_ch4
        ...
        #marginal_heat_interior[:,i]   = sneasy_marginal[:doeclim, :heat_interior] + H0 + ar1_heat
        #marginal_oceanco2[:,i]        = sneasy_marginal[:ccm, :atm_oc_flux] + norm_oceanco2
        #marginal_lifech4[:,i]         = sneasy_marginal[:ohcyclehector, :TOH]

        #Print iteration number to track progress.
        #println("Completed Hector model evaluation ", i, " of ", number_samples)
        #println(i)
    end
...
Conf_lvl_2 = 0.98
        ...
        conf_base_temp     = confidence_int(years, transpose(base_temp), Conf_lvl_1, Conf_lvl_2)
        conf_base_co2      = confidence_int(years, transpose(base_co2), Conf_lvl_1, Conf_lvl_2)
        conf_base_ch4      = confidence_int(years, transpose(base_ch4), Conf_lvl_1, Conf_lvl_2)
        conf_base_heatint  = confidence_int(years, transpose(base_heat_interior), Conf_lvl_1, Conf_lvl_2)
        conf_base_oceanco2 = confidence_int(years, transpose(base_oceanco2), Conf_lvl_1, Conf_lvl_2)
        #conf_base_lifech4  = confidence_int(years, transpose(base_lifech4), Conf_lvl_1, Conf_lvl_2)
...
temp_plot = spaghetti(transpose(base_temp), conf_base_temp, calibration_data, "hadcrut_temperature", "red", 0.1, 1765, 2300, 400, [1850,2017], [-0.5,1.5], "Year", "Temp", "Ham")
...
ch4_plot = spaghetti(transpose(base_ch4), conf_base_ch4, calibration_data, "obs_ch4inst", "dodgerblue", 0.2, 1765, 2300, 900, [1850,2017], [500,2000], "Year", "Temp", "Ham")
R"""
...
co2_plot = spaghetti(transpose(base_co2), conf_base_co2, calibration_data, "obs_co2inst", "dodgerblue", 0.1, 1765, 2300, 400, [1850,2017], [250,400], "Year", "Temp", "Ham")
R"""

...
include("calibration/sneasy_magicc/assim.jl")
...
function run_mcmc_m(mcmc_file::String, n_steps::Int64, burn_in::Int64, save_results::Bool, save_full_chain::Bool)
    ...
    parnames = ["S","kappa","alpha","Q10","beta","eta","T0","H0","CO20","sigma.T","sigma.H","sigma.CO2.inst","sigma.CO2.ice","rho.T","rho.H","rho.CO2.inst", "TAUINIT", "signma.ch4.inst", "rho.ch4.inst","signma.ch4.ice", "rho.ch4.ice", "CH4_1", "CH4_2", "CH4_Nat", "T_Soil", "T_Strat", "N2O_0", "F2x_CO₂", "rf_scale_CH₄"]
    ...
    ftol_rel!(opt, 1e-8)
 	#lower_bounds!(opt, Float64[0,0,0,1,0,0,-Inf,-100,280,1e-10,1e-10,1e-10,1e-10,0,0,0,0,1e-10,0, 1e-10,0,697, 238, 96, 100, 266])
   	...
   	lower_bounds!(opt, Float64[0,0,0,1,0,0,-Inf,-100,276,1e-10,1e-10,1e-10,1e-10,0,0,0,0,1e-10,0, 1e-10,0,696, 150, 96, 100, 265, 2.968, 0.72])
...
chain, mcchain_object = mcmc_stuff(5_000_000, log_post, mcmc_starting_point, mcmc_step)
    ...
    if save_results
        ...
        output_folder = joinpath("calibration/sneasy_magicc/mcmc_results", mcmc_file)
        mkdir(output_folder)

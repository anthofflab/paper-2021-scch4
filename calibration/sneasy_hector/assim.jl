...
function sneasy_load_data()
   ...
   # df = DataFrame(year=df[:year], co2=df[:FossilCO2]+df[:OtherCO2], ch4_emissions=df[:CH4], rf_aerosols=df[:aerosol_direct]+df[:aerosol_indirect], rf_other=df[:ghg_nonco2]+df[:solar]+df[:volcanic]+df[:other]-df[:ch4_rf])
    df = DataFrame(year=df[:year], co2=df[:FossilCO2]+df[:OtherCO2], ch4_emissions=df[:CH4], nox_emissions=df[:NOx], co_emissions=df[:CO], nmvoc_emissions=df[:NMVOC], rf_aerosols=df[:aerosol_direct]+df[:aerosol_indirect], rf_other=df[:ghg_nonco2]+df[:volcanic]+df[:solar]+df[:other]- df[:CH4_RF] - df[:ch4_h20_rf] - df[:TROPOZ_RF])
...
function log_pri(p)
	...
	#prior_S = Uniform(0.00000001, 10.0)
    ...
    #IPCC = 120 year lifetime +/- 24.
    prior_Tsoil = Uniform(96, 144)
    # Same description as TSoil, but IPCC has central value of 150 years ± 50.
    prior_Tstrat = Uniform(100, 200)
	...
	lpri = -Inf
	if S>0.
        #lpri = logpdf(prior_S1, S) + logpdf(prior_S2, S) + logpdf(prior_κ, κ) + logpdf(prior_α, α) + logpdf(prior_Q10, Q10) + logpdf(prior_beta, beta) + logpdf(prior_eta, eta) + logpdf(prior_T0, T0) + logpdf(prior_H0, H0) + logpdf(prior_CO20, CO20) + logpdf(prior_σ_temp, σ_temp) + logpdf(prior_σ_ocheat, σ_ocheat) + logpdf(prior_σ_co2inst, σ_co2inst) + logpdf(prior_σ_co2ice, σ_co2ice) + logpdf(prior_ρ_temp, ρ_temp) + logpdf(prior_ρ_ocheat, ρ_ocheat) + logpdf(prior_ρ_co2inst, ρ_co2inst) + logpdf(prior_TOH0, TOH0) +logpdf(prior_σ_ch4inst, σ_ch4inst) + logpdf(prior_ρ_ch4inst, ρ_ch4inst)  +logpdf(prior_σ_ch4ice, σ_ch4ice) + logpdf(prior_ρ_ch4ice, ρ_ch4ice) + logpdf(prior_CH4_0, CH4_0) + logpdf(prior_CH4_Nat, CH4_Nat) + logpdf(prior_Tsoil, Tsoil) + logpdf(prior_Tstrat, Tstrat) + logpdf(prior_N2O_0, N2O_0) + logpdf(prior_F2x_CO₂, F2x_CO₂) + logpdf(prior_rf_scale_CH₄, rf_scale_CH₄)
...
function construct_log_post(f_run_model; start_year=1765, end_year=2017, assim_temp=true, assim_ocheat=true, assim_co2inst=true, assim_co2ice=true, assim_ocflux=true, assim_ch4inst = true, assim_ch4ice=true)
	...
	function sneays_log_lik(p)
        ...
        f_run_model(model_co2, model_ch4, model_ocflux, model_temperature, model_ocheat,
            S, κ, α, Q10, beta, eta, TOH0, CH4_Nat, Tsoil, Tstrat, CO20, T0, H0, CH40, N2O_0, F2x_CO₂, rf_scale_CH₄)
